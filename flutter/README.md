# Misar.Blog Flutter SDK

Official Flutter client for the Misar.Blog developer API, with secure key storage and an embed widget for iOS, Android, macOS and Web.

## Install

```bash
flutter pub add misarblog_flutter
```
...
// Direct key, or MisarBlogClient.withSecureStorage() to load from the keychain.
final blog = MisarBlogClient(apiKey: 'mbk_...');

final me = await blog.account.profile();
final thread = await blog.comments.list('article-id', limit: 50);

try {
  await blog.ai.complete(prompt: 'Draft an intro paragraph');
} on MisarBlogPlanLimitException catch (e) {
  // Route the user to checkout rather than showing a bare failure.
...
The last two raise ``MisarBlogPlanLimitException`` rather than a generic error, carrying the

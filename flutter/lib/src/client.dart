...
/// Flutter client for the Misar.Blog developer API.
///
...
class MisarBlogClient {
  ...
  static Future<MisarBlogClient> withSecureStorage({
    ...
    if (apiKey == null || apiKey.isEmpty) {
      throw StateError(
        ...
        'Call SecureKeyStore().saveApiKey("mbk_...") first.',
      );
    }
      ...
      try {
        ...
        switch (method) {
          ...
          case 'DELETE':
            ...
            break;
          default:
            throw MisarBlogException(0, 'Unsupported HTTP method: $method');
        }
        ...
        if (planLimitBody != null) {
          throw MisarBlogPlanLimitException.from(
        ...
        final message = (errBody?['error'] as String?) ??
            ...
            'HTTP $status';
        throw MisarBlogException(status, message, errBody);
      } on MisarBlogException {
        rethrow;
      } on SocketException catch (e) {
        if (isFinal) throw MisarBlogNetworkException(e.message);
        await _backoff(attempt);
        attempt++;
        continue;
...
// ---------------------------------------------------------------------------
// Resources (identical surface to the pure-Dart SDK)
// ---------------------------------------------------------------------------
...
class AnalyticsResource extends _Resource {
  ...
  /// `GET /analytics`
  Future<Analytics> get({int? days}) async => Analytics(
      await _client.request('GET', '/analytics', query: {'days': days}));
}
